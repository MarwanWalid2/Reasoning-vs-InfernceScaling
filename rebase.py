...
class LocalModelManager:
    """Manages local model inference for both policy and reward models"""
    def __init__(self, config: Dict[str, Any]):
        logger.info(f"Loading policy model from {config['policy_path']}")
        self.policy_tokenizer = AutoTokenizer.from_pretrained(config['policy_path'])
        ...
        logger.info(f"Loading reward model from {config['reward_path']}")
        ...
        # Calculate FLOPs per token for both models
    ...
    def generate_step(self, state: ModelState, max_tokens: int, 
        ...
        """Generate next step with the policy model"""
        try:
            start_time = time.perf_counter()
            # Format prompt for mathematical context
            ...
            generation_flops = self.policy_flops_per_token * max_tokens
            ...
            outputs = self.policy_model.generate(
                **inputs,
                ...
                stopping_criteria=self._create_stopping_criteria(stop_str),
                early_stopping=True,
            )
            ...
            # Add step tag and calculate reward
            step_text = new_text + " ки"
            score = self._calculate_reward(step_text)

            # Track time and total FLOPs
    ...
    def _calculate_reward(self, text: str) -> float:
        """Calculate reward score for generated text"""
        try:
            # Format input with step tags if not present
            if 'ки' not in text:
            ...
            with torch.no_grad():
                ...
                # Return average step score
                return step_scores.mean().item()
        except Exception as e:
            logger.error(f"Error in _calculate_reward: {str(e)}")
            return 0.0

    def _create_stopping_criteria(self, stop_str: str) -> StoppingCriteriaList:
...
class Tree:
    """Search tree for exploring solution paths"""
    ...
    def expand(self, node: TreeNode, wid: int):
        ...
        for fork in forks:
            ...
            # Only add states that have valid scores
            if new_state.scores and new_state.scores[-1] > 0.1:  # Threshold for valid steps
...
if __name__ == "__main__":
    ...
    config = {
        ...
        "policy_path": "mistralai/Mistral-7B-v0.1",
    ...
    try:
        ...
        print("Active processes:", [p.info for p in psutil.process_iter(attrs=['pid', 'name'])])
    except Exception as e:
